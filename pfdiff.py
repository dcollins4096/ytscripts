...
class udiff():
    ...
    def __init__(self,ds1,ds2=None,sticky=True,**kwargs):
        """Set up comparisons between *uber1* and *uber2*.  If *uber2* is
        ...
        *sticky* causes arguments to the __call__ method to replace existing
        ...
        self.p['frames']=copy.copy(self.uber1.frames)
        self.p['grids']=[1]
        self.p['fields']=copy.copy(self.uber1.fields)
            ...
            for g in grids:
                ...
                for field in fields:
                    ...
                    if self.p['grid_direct']:
                        g1_full= self.uber1.grid(n1,grid1,field1)
                        g2_full = self.uber2.grid(n2,grid2,field2)
                    else:
                    ...
                    stat(g1,"g1: n1 = %d g1 = %d f = %s %s"%(n1,grid1,field1,self.uber1.name))
                    stat(g2,"g2: n2 = %d g2 = %d f = %s %s"%(n2,grid2,field2,self.uber2.name))
                    ...
                    if output:
                        ...
                        for x in interogate_range: #range(g1.shape[0]):
                            ...
                            plave(g1[subset],self.output_format%(output_prefix,\
                                                           n1,grid1,field1,output,x,self.uber1.outname))
                            plave(g2[subset],self.output_format%(output_prefix,
                                                           n2,grid2,field2,output,x,self.uber2.outname))
                            diff_out = field1
                            if field1 != field2:
                                diff_out += field2
