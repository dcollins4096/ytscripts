...
class Clump(object):
    ...
    def find_children(self, min, max = None):
        ...
        contour_info = amods.level_sets.identify_contours(self.data, self.field, min, max,
                                         self.cached_fields)
        for cid in contour_info:
            ...
            self.children[-1].contour_min = min

    def __reduce__(self):
        return (_reconstruct_clump, 
                (self.parent, self.field, self.min, self.max,
                 self.function_value, self.children, self.data,self.CenterOfMass, self.stuff,self.function,self.endpoint))

    def __getitem__(self,request):
        return self.data[request]
def _reconstruct_clump(parent, field, mi, ma, function_value, children, data,
                       CenterOfMass,stuff,function=None,endpoint=-1):
    obj = object.__new__(Clump)
    if iterable(parent): parent = parent[1]
    if children is None: children = []
    obj.parent, obj.field, obj.min, obj.max, obj.function_value, \
       obj.children, obj.function, obj.CenterOfMass,obj.endpoint = parent, field, mi, ma, function_value, children, function,\
       CenterOfMass, endpoint
    obj.stuff = stuff
...
def find_dendrogram(clump,min,max,d_clump,level=0):
    if min >= max: return
    clump.find_children(min)
    if clump.children != None:
        for XXX,child in enumerate(clump.children):
            print "Investigating %d/%d children, level %d." % (XXX,len(clump.children),level)
            find_dendrogram(child, min*d_clump,max,d_clump,level=level+1)
...
def make_dendrogram(clump):
    """Recursively make a dendrogram from *clump*.
    Forces all clumps to have two or zero children.
    Cleans up fluctuations (not finished)"""
