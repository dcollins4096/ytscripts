    ...
    *scale* is the data units per arrow length unit using *scale_units*
    ...
    def __call__(self, plot):
        ...
        if self.weight == 'N':
            ...
            print "Weight I (rly)", self.scale, 'b'
        print "WEIGHT WTF hax2", self.weight, self.scale
        ...
        Ix = np.sqrt(pixX**2+pixY**2)
        cut = np.log10(Ix) > -1
        self.kludge_my_package['Ix']=Ix
        if 1:
            X=X[cut]
            Y=Y[cut]
            pixX=pixX[cut]
            pixY=pixY[cut]
            print "CUT", cut.flatten().sum()
        if self.normalize:
            nn = np.sqrt(pixX**2 + pixY**2)
            pixX /= nn
            pixY /= nn
        plot._axes.quiver(X,Y, pixX, pixY, scale=self.scale, scale_units=self.scale_units, headlength=0, headwidth=1, width=2e-3,
                          color='r')
        plot._axes.set_xlim(xx0,xx1)
        plot._axes.set_ylim(yy0,yy1)
