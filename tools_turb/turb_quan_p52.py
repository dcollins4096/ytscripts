...
import p49_fields
reload(p49_fields)
from multi_imshow import plotter, plotter2
import yt
...
class quan_box():
    def __init__(self,car=None, plot_format='png'):
        ...
        self.all_fields +=['Bx','By','Bz','Bfield_strength','AlfMach','beta','AlfvenSpeed','frames','B2sphere','volume']
...
#            self.plot_eebb()
    ...
    def quadratic(self,car, frame, tdyn=1, ds=None):
        ...
        if UseMHD:
            sph = ds.sphere([0,0,0],(20000,'km'))
            volume = sph.quantities['TotalQuantity']('cell_volume')
            self.stuff['B2sphere'].append( (sph.quantities['WeightedAverageQuantity']('magnetic_energy','cell_volume')).in_units('erg/cm**3'))
            self.stuff['volume'].append(volume)
